...
function playSlice() {
  ...
  const g2 = AC.createGain(); g2.gain.setValueAtTime(0.3, AC.currentTime);
...
class Particle {
  ...
  update() {
    this.x += this.vx; this.y += this.vy; this.vy += .35;
    this.life -= .055; if (this.life <= 0) this.alive = false;
  }
  draw() {
    ctx.save();
...
class Popup {
  ...
  update() { this.y-=1.8; this.life-=.025; if(this.life<=0) this.alive=false; }
  draw() {
    ctx.save();
    ctx.globalAlpha = Math.max(0, this.life);
...
class Fruit {
  constructor() {
    ...
    this.sliced= false;
    // halves
    this.h = [{x:this.x,y:this.y,vx:this.vx-5,vy:this.vy-2},
              {x:this.x,y:this.y,vx:this.vx+5,vy:this.vy-2}];
  }
  update() {
    if (this.sliced) {
      this.h.forEach(p => { p.x+=p.vx; p.y+=p.vy; p.vy+=this.g; });
      if (this.h[0].y > H+80 && this.h[1].y > H+80) this.alive = false;
    } else {
      this.x+=this.vx; this.y+=this.vy; this.vy+=this.g; this.rot+=this.rotS;
  ...
  _drawFull() {
    ...
    ctx.fillStyle = this.cfg.color; ctx.fill();
    ctx.strokeStyle = 'rgba(255,255,255,.4)'; ctx.lineWidth = 2; ctx.stroke();
    // shine
    ctx.beginPath(); ctx.arc(-this.r*.3,-this.r*.3,this.r*.22,0,Math.PI*2);
  ...
  _drawHalf(i) {
    ctx.beginPath();
    ctx.arc(0, 0, this.r, i===0?Math.PI:0, i===0?Math.PI*2:Math.PI);
    ctx.fillStyle = this.cfg.inner; ctx.fill();
    ctx.strokeStyle = 'rgba(255,255,255,.3)'; ctx.lineWidth=1.5; ctx.stroke();
    // juice line
    ctx.beginPath();
    ctx.moveTo(i===0?this.r:-this.r, 0); ctx.lineTo(i===0?-this.r:this.r, 0);
    ctx.strokeStyle = this.cfg.color; ctx.lineWidth=3; ctx.stroke();
  }
}
...
class Bomb {
  constructor() {
    ...
    this.vx=spd*Math.sin(ang); this.vy=-spd; this.g=.45;
    this.r=38; this.alive=true; this.rot=0;
  }
  update() {
    this.x+=this.vx; this.y+=this.vy; this.vy+=this.g; this.rot+=2;
  ...
  draw() {
    ...
    // shine
    ctx.beginPath(); ctx.arc(-this.r*.3,-this.r*.3,this.r*.2,0,Math.PI*2);
    ...
    ctx.beginPath(); ctx.arc(4+Math.sin(t)*2,-this.r-27+Math.cos(t)*2,5,0,Math.PI*2);
    ctx.fillStyle=`hsl(${40+Math.sin(t)*20},100%,60%)`; ctx.fill();
    // label
    ctx.font='bold 14px Nunito'; ctx.fillStyle='rgba(255,255,255,.8)';
    ctx.textAlign='center'; ctx.textBaseline='middle';
...
let fruits=[], bombs=[], particles=[], popups=[];
let gameRunning=false, gameOver=false;
let spawnTimer=0, frameCount=0;
let combo=0, comboTimer=0;
let bestScore = parseInt(localStorage.getItem('fnBest')||'0');

function resetGame() {
  score=0; lives=5; level=1;
  fruits=[]; bombs=[]; particles=[]; popups=[];
  trail=[]; spawnTimer=0; frameCount=0;
...
function checkSlice(obj) {
  ...
  for (let i=1; i<trail.length; i++) {
    ...
    const t = Math.max(0,Math.min(1,((obj.x-p1.x)*dx+(obj.y-p1.y)*dy)/(dx*dx+dy*dy)));
...
function trailSpeed() {
  ...
  const p1=trail[trail.length-3], p2=trail[trail.length-1];
...
function gameLoop(ts) {
  ...
  if (spawnTimer >= spawnRate) {
    ...
    if(Math.random() < .12+level*.03) bombs.push(new Bomb());
  }
  ...
  // HUD score on canvas
  ctx.font='900 48px Bangers'; ctx.fillStyle='rgba(255,255,255,.9)';
  ctx.textAlign='left'; ctx.textBaseline='top';
  ctx.shadowColor='rgba(0,0,0,.8)'; ctx.shadowBlur=12;
  ctx.fillText(`SCORE  ${score}`, 20, 16);
  ctx.font='700 20px Bangers'; ctx.fillStyle='rgba(255,255,255,.4)';
...
function setFinger(nx, ny) {
  ...
  fruits.forEach(f=>{
    if(!f.sliced && checkSlice(f) && speed>12){
      ...
      comboTimer=55;
      playSlice();
      for(let i=0;i<14;i++) particles.push(new Particle(f.x,f.y,f.cfg.inner));
      popups.push(new Popup(f.x,f.y-20,'+1','#30d158',30));

      if(combo>=3){
        const bonus=combo-2;
        score+=bonus;
        popups.push(new Popup(f.x,f.y-60,`COMBO x${combo}! +${bonus}`,'#ffd60a',36));
  ...
  bombs.forEach(b=>{
    if(checkSlice(b) && speed>12){
      ...
      combo=0; comboTimer=0;
      playBomb();
      for(let i=0;i<20;i++) particles.push(new Particle(b.x,b.y,'#ff2d55'));
      popups.push(new Popup(b.x,b.y,'💥 -1 LIFE','#ff2d55',34));
      document.getElementById('bomb-flash').classList.add('flash');
      setTimeout(()=>document.getElementById('bomb-flash').classList.remove('flash'),120);
      updateHUD();
      if(lives<=0) triggerGameOver();
    }
  });
...
function triggerGameOver() {
  ...
  if(score>bestScore){ bestScore=score; localStorage.setItem('fnBest',score); }
